package com.example.procard.ui.screens.descanso
...
fun DescansoScreen() {
    ...
    Column(Modifier.fillMaxSize()) {
        AppHeader(user = user ?: UserProfile("u-0", "", null), title = NavRoute.Descanso.title, subtitle = NavRoute.Descanso.subtitle)
        if (state.loading) LinearProgressIndicator(modifier = Modifier.fillMaxWidth())
        state.error?.let { ErrorBanner(it) { } }
        when {
            state.empty -> EmptyState("Sin registros de descanso.", "Agregar registro") {}
            !state.loading && state.error == null -> Text("Contenido de Descanso", Modifier.padding(16.dp))
        }
    }
}