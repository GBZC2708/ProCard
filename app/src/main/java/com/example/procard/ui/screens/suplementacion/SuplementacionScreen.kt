...
fun SuplementacionScreen() {
    ...
    Column(Modifier.fillMaxSize()) {
        AppHeader(user = user ?: UserProfile("u-0", "", null), title = NavRoute.Suplementacion.title, subtitle = NavRoute.Suplementacion.subtitle)
        if (state.loading) LinearProgressIndicator(modifier = Modifier.fillMaxWidth())
        state.error?.let { ErrorBanner(it) { } }
        when {
            state.empty -> EmptyState("Sin suplementos asignados.", "Agregar suplemento") {}
            !state.loading && state.error == null -> Text("Contenido de Suplementación", Modifier.padding(16.dp))
        }
    }
}